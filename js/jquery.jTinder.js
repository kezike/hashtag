...
;(function ($, window, document, undefined) {
	...
	Plugin.prototype = {
		...
		/* panes is an object eq selects item at that index*/
		...
		next: function () {
			return this.showPane(current_pane - 1); /*going backwords */
		},

		/* here is actual work to backend I believe*/
		dislike: function() {
			panes.eq(current_pane).animate({"transform": "translate(-" + (pane_width) + "px," + (pane_width*-1.5) + "px) rotate(-60deg)"}, $that.settings.animationSpeed, function () {
				// this does the minus one at bottom
		...
		like: function() {
			panes.eq(current_pane).animate({"transform": "translate(" + (pane_width) + "px," + (pane_width*-1.5) + "px) rotate(60deg)"}, $that.settings.animationSpeed, function () {
				//this part is just adding the plus one for index
